...
class EDF_CD(Scheduler):
    ...
    def on_activate(self, job):
        if all(self.R):
            job.priority = self.modes.conditional_dmp(self.R, job.task.id, self.last_activated_jobs)
        else:
    ...
    def schedule(self, cpu):
        ...
        if self.ready_list:
            ...
            key = lambda x: (0 if not x.running else 1,
                             self.modes.conditional_dmp(self.R, x.running.task.id,
                                                        self.last_activated_jobs) if x.running and all(self.R) else 1,
            ...
            if active_jobs:
                key = lambda x: (self.modes.conditional_dmp(self.R, x.task.id, self.last_activated_jobs)
                                 if all(self.R) else 0,
                                 x.priority)
                job = min(active_jobs, key=key)


                if (cpu_min.running is None or
                        self.modes.conditional_dmp(self.R, cpu_min.running.task.id,
                                                   self.last_activated_jobs) > self.modes.conditional_dmp(self.R, job.task.id,
